...
class Translator():
    ...
    def charEncode(self, character, b):
        '''
    ...
    def charDecode(self, character, b):
        '''
    ...
    def Encode(self, msg):
        ...
        for i in range(len(msg)):
            character = msg[i]
            encodedChar = self.charEncode(character, base)
            # print(encodedChar)
    ...
    def Decode(self, msg):
        ...
        for i in range(len(splitMsg)):
            decodedMsg += self.charDecode(splitMsg[i], base)
    ...
    def translate(self, msg):
        if msg[0] == "*":   #Determines if the message is already encoded
            self.Decode(msg)
        else:
            self.Encode(msg)
        return _void
...
E1.insert(0, "TEST")
ttk.Button(frm, text="Translate", command=trans(E1.get())).grid(column=1, row=1)
ttk.Button(frm, text='Quit', command=frm.quit).grid(row=3, column=0, pady=4)
root.mainloop()